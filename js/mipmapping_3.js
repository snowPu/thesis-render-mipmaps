...
async function updateMesh(which, ts, met, geo) {
    ...
    // let ind = scene.children.map(ch => ch.name).indexOf(which);
    // if (ind > -1) {
    //     if (scene.children[ind].material) { scene.children[ind].material.dispose(); }
    //     if (scene.children[ind].geometry) { scene.children[ind].geometry.dispose(); }
    //     scene.remove(scene.children[ind]);
    // }
    let scene = scene1;
    if (which === 'second') {
        scene = scene2;
